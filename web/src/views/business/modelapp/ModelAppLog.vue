    ...
    <div class="watch-log-page">
        ...
        <div class="log-info defaultBg">
            ...
            <div class="log-info__main">
                <div style="top: 28px;width: 42px;height: 42px;" class="scrollBottom pointer" title="返回" @click="back">
...
import { Get_App_Logs } from "@/api/business/application/application"
...
export default {
    ...
    data() { // 定义页面变量
        return {
            ...
            // 表单字段
            options: [
                { type: 'text', dataType: 'int', name: fieldObj.lineNum, label: '展示行数(建议值)', value: 200 },
    ...
    methods: { // 定义函数
        ...
        getLog() {
            ...
            let params = { id: this.instanceId, logType: '0', size: _lineNum, flushTime: _refreshTimeGap }
            if(this.$route.query.modelSyncFlag){
                params.modelSyncFlag = this.$route.query.modelSyncFlag
            }
            let _req = Get_App_Logs(params)
            _req.then(res => {
                // 只要成功就设置当前失败次数为0
                ...
                let resCode = encodeURIComponent(res.message)
                // 定义编码后的换行符（等下要限制输出行数）
                let lineFeedCode = '%0A'
                // 根据换行符获取所有行数据数组
        ...
        back() {
            this.pushPage({
                path: '/module/stander/list/' + this.$route.query.reload + '/placeholder',
                title: 'path'
            })
        }
    },
