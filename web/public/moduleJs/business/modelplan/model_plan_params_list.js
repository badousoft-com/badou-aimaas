export default {
    ...
    paramsBeforeRequest: function (params) {
        let url = `${this.BASEURL}/project/maas/tuningplanparams/tuningplanparamsedit/getSelectOption`
    ...
    searchFixedRightSlot: function () {
        return Promise.resolve((h, context) => (
            /** start-希望展示在页面上的内容的编写 */
            // 这里的内容区的编写要符合render jsx语法，详细可查看【字段渲染与事件】中对内容编写的要求
            <bd-button icon="compass" type="primary" class="marginB" nativeOn-click={(e) => {
                // this.$message.success('edit')
                window.open('https://llamafactory.readthedocs.io/zh-cn/latest/advanced/arguments.html', '_blank')
                e.stopPropagation() // 阻止事件冒泡，避免表格行被选中
            }}>llamafactory参数页面</bd-button>
            /** end-希望展示在页面上的内容的编写 */
        ))
    },
    renderField: {
        ...
        value: {
            render: function (h, context, row, column, cellValue, index, fieldObj, scope) {
                ...
                if (selection) {
                    ...
                    return (
                        <el-input vModel={row.value} class="primary" on-change={(e) => {
                            this.post('/project/maas/tuningplanparams/tuningplanparamsedit/updateValue', {
                                id: row.id,
                                newValue: e
                            }).then((res) => {
