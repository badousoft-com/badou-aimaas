package com.badou.project.maas.problemdata.service;

import java.io.Serializable;
import java.util.List;

import com.badou.brms.base.support.spring.IBaseSpringService;
import com.badou.project.maas.maasfile.model.MaasFileVideoEntity;
import com.badou.project.maas.problemdata.model.ProblemDataPictureEntity;


/**
 * @author badousoft
 * @date 2025-03-26 10:13:00.13
 *  图片样本集 service接口
 **/
public interface IProblemDataPictureService extends IBaseSpringService<ProblemDataPictureEntity, Serializable> {
    /**
     * 批量更新启用状态 启用变成停用 停用变成启用
     * @param datas 数据列表
     */
    void updateStatus(List<ProblemDataPictureEntity> datas);

}