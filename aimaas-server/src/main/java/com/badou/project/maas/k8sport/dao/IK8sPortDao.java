package com.badou.project.maas.k8sport.dao;

import com.badou.brms.base.support.hibernate.IBaseHibernateDAO;
import com.badou.project.maas.modelapp.model.ModelAppEntity;

import java.io.Serializable;

/**
 * @author badousoft
 * @created 2024-05-08 17:03:35.357
 * @todo 微调模型评价管理dao接口
 */
public interface IK8sPortDao extends IBaseHibernateDAO<ModelAppEntity, Serializable> {

    /**
     * 新增一条数据
     * @param port
     * @return
     * @throws Exception
     */
    int deleteNew(int port) throws Exception;

    /**
     * 新增一条数据
     * @param port
     * @return
     * @throws Exception
     */
    int insertNew(int port) throws Exception;

    /**
     * 获取最大的端口号码 默认是从35000开始
     * @return
     * @throws Exception
     */
    Integer getMax() throws Exception;
}
